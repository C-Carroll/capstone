...
router.post("/album/new", validateAlbum ,requireAuth, async (req, res) => {
    ...
    else {return res.status(403).json({message: 'User must be an artist to add music'})}
})
...
router.post('/song/:albumId', validateSong, requireAuth, async (req, res) => {
    ...
    if (artistSearch !== null) {
        ...
        }else {return res.status(403).json({message: 'User must be creator of album to add music'})}
    }else{return res.status(403).json({message: 'User must be an artist to add music'})}
})
