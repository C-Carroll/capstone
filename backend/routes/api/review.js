...
router.delete("/delete/:reviewId", requireAuth, async (req, res) => {
  ...
  } else if (findRev.userId !== req.user.id) {
...
router.post("/new/:albumId", validRev, requireAuth, async (req, res) => {
  ...
  if (findAlbum.Artist.userId === req.user.id) {
    res.status(403).json({ message: "Cant Review your own album" });
  } else if (revCheck) {
    res.status(403).json({ message: "cant review album more than once" });
  } else {
    let newRev;
    const { rating, ratingDescription } = req.body;
    try {
